...
test.describe('Rectangle tool', () => {
  ...
  test('should change cursor to move immediately when clicking to select rectangle', async ({
    page,
  }) => {
    await page.click('[data-tool="rectangle"]');
    ...
    // Click on rectangle edge to select (without moving mouse afterward)
    const midY = (startY + endY) / 2;
    await page.mouse.click(startX, midY);
    await page.waitForTimeout(50);
